...
async function login (req, res) {
    ...
    if (userName.trim() == "" || pass.trim() == ""){
        return res.status(400).json({status:"Nome de usuário ou senha não inseridos"})
    }
    ...
    if (!user){
        return res.status(404).json({status:"Nome de usuário ou senha não combinam!"})
    }
    ...
    try{
        ...
        console.log(error);
        return res.status(500).json({status:"Ocoerreu um erro ao realizar a atenticação!"})
    }
}
...
async function emailConfirmation2 (req, res){
    ...
    try{
        ...
        return res.status(204).json({"status":"Mensagem de confirmação enviada com sucesso!"})
    }
    catch(err){
        console.log(err);
    }
